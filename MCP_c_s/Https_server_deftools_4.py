...
async def echo_tool(text:str) -> list[types.ContentBlock]:
    """echo the text"""
...
def main() -> int:
    ...
    async def fetch_tool(name: str, arguments: dict) -> list[types.ContentBlock]:
        if name not in tools_collector:
            raise ValueError(f"Unknown tool: {name}")
        tool_info = tools_collector[name]
        func = tool_info["func"]
        args = tool_info["args"]
        if set(args) != set(arguments.keys()):
            raise ValueError("required argument:" + str(args))
